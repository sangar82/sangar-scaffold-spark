...
class ".ucfirst($this->controller_name)." extends CI_Controller
{
...
$data .= "
		...
		if (!\$id or !\$data['".$this->relational_field."']){
			\$this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
			redirect(\"".$this->controller_name."/\".\$data['".$this->relational_field."']);
		}
";
}
else
...
";
		...
		redirect(\"".$this->controller_name."/\"".( ($this->there_is_a_relational_field) ? ".\$".$this->model_name."->".$this->relational_field."" : "" ).");
	}
